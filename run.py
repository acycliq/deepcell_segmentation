...
def get_jpg(i):
    ...
    jpg_page = os.path.join(BOUNDARIES_JPG_DIR, 'dapi_image_rescaled_zxyc%s.jpg' % str(i).zfill(4))
    if os.path.isfile(jpg_page):
        return jpg_page
    else:
        return os.path.join(FRAMES_JPG_DIR, 'dapi_image_rescaled_zxyc%s.jpg' % str(i).zfill(4))
...
if __name__ == "__main__":
    img_path = os.path.join(ROOT_DIR, 'data', '3D_dapi', 'dapi_image_rescaled_zxyc.tif')
    main(img_path)
    logger.info('Done')